...
class EaDesign_PdfGenerator_Model_Entity_Pdfgenerator extends EaDesign_PdfGenerator_Model_Entity_Abstract
{
    ...
    {
        if ($this->productId) {
            return EaDesign_PdfGenerator_Model_Pdfgenerator::PRODUCTTEMPLATE;
        }
        if ($this->orderId) {
    ...
    public function loadPdf()
    {
        ...
        $pdf = new Mpdf_Mpdfstart('+aCJK', $this->pdfPaperFormat(), 8, '', $left, $right, $top, $bottom);
        $pdf->shrink_tables_to_fit = 0;
        // Comment from previous company. May be useful in debug:
    ...
    {
        if ($this->productId) {
            return array(0, Mage::app()->getStore()->getId());
        }
        if ($storeId = $this->getTheSource()->getStore()->getId()) {
            return array(0, $storeId);
        }
        if ($storeId = $this->getTheSource()->getOrder()->getStore()->getId()) {
            return array(0, $storeId);
        }
        return array(0);
    }
    ...
    public function getFilteredCollection()
    {
        try {
            if ($this->templateId) {
                ...
                /* need to add the source bether */
                $pdfGeneratorTemplate = $this->getPdfCollection()
                    ->addFieldToSelect('*')
                    ->addFieldToFilter('pdft_is_active', 1)
                    ->addFieldToFilter('pdft_type', $this->getCollectionFilter())
    ...
    public function getTheTemplateBodyWithItems()
    {
        ...
        if ($this->productId) {
            $templateToProcessForItems = $this->getBody();
            return $templateToProcessForItems;
        }
        /* need to add the source bether */
        if ($this->getTheSource()->getOrder()) {
            $orderObj = $this->getTheSource()->getOrder();
        } else {
            $orderObj = $this->getTheSource();
        }
        $items = Mage::getModel('eadesign/entity_items')
            ->setSource($this->getTheSource())->setOrder($orderObj);
