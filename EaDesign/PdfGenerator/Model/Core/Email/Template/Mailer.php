...
class EaDesign_PdfGenerator_Model_Core_Email_Template_Mailer extends Mage_Core_Model_Email_Template_Mailer
{
    ...
    public function dispatchEadesignPdfEvent($emailTemplate)
    {
        ...
        if ($templateParams['order']) {
            $theEntity = $templateParams['order'];
        }

        if ($templateParams['invoice']) {
            $theEntity = $templateParams['invoice'];
        }

        if ($templateParams['creditmemo']) {
            $theEntity = $templateParams['creditmemo'];
        }

        if ($templateParams['shipment']) {
            $theEntity = $templateParams['shipment'];
        }


        if ($templateParams['invoice'] || $templateParams['creditmemo'] || $templateParams['shipment'] || $templateParams['order']) {
            Mage::dispatchEvent(
