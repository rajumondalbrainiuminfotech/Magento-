...
class EaDesign_PdfGenerator_Block_Adminhtml_Template_Pdf_Edit_Tabs_General extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        if ($model->getData('pdft_type') == 'productpdftemplate') {
            $fieldset->addField('pdft_attribute_set', 'select', array(
                'label' => Mage::helper('cms')->__('Attribute set'),
                'title' => Mage::helper('cms')->__('Attribute set'),
                'name' => 'pdft_attribute_set',
                'required' => true,
        ...
        $fieldset->addField('pdft_type', 'select', array(
            ...
            'options' => array(
                /* review here */
                $this->getThePdfType() => $this->getThePdfType(),
            ),
            'value' => 1,
            'required' => true,
            'readonly' => true,
    ...
    {
        if ($pdfType = $this->getRequest()->getParam('type')) {
            return $pdfType;
        }
        $model = mage::registry('pdfgenerator_template');
        return $model->getData('pdft_type');
    }
